...
public class ConstructBinaryTreeFromPreorderAndInorderTraversal {
    ...
    private TreeNode buildTree(int[] preorder, int pBeg, int pEnd, int[] inorder, int iBeg, int iEnd) {
        if (pBeg > pEnd)
            return null;
        int val = preorder[pBeg];
        TreeNode node = new TreeNode(val);
        int iIndex = findIndex(inorder, iBeg, iEnd, val);
        int leftChildrenLength = iIndex - iBeg;
//        int rightChildrenLength = iEnd - iBeg - leftChildrenLength;
        node.left = buildTree(preorder, pBeg + 1, pBeg + leftChildrenLength, inorder, iBeg, iIndex - 1);
        node.right = buildTree(preorder, pBeg + leftChildrenLength + 1, pEnd, inorder, iIndex + 1, iEnd);
        return node;
    }
    
    private int findIndex(int[] inorder, int beg, int end, int val) {
